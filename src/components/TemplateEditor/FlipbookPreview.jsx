...
const FlipbookPreview = ({ pages, pageName = "Name of the Book", onClose, isMobile = false, isDoublePage }) => {
                ...
                if (eventType === 'click') {
                  ...
                  window.parent.postMessage({
                    ...
                    data: {
                      ...
                      page: parseInt(window.frameElement?.parentNode?.dataset?.pageNumber || '1')
                    }
                  }, '*');
                }
      ...
      if (event.data && event.data.type === 'flipbook-spread-zoom') {
        ...
        if (current.active && current.elementId === elementId) {
          ...
          if (iframe) {
            ...
            const elCX = iframeRect.left + rect.left + rect.width / 2;
            const elCY = iframeRect.top + rect.top + rect.height / 2;

            // Viewport center
            const viewCX = window.innerWidth / 2;
            const viewCY = window.innerHeight / 2;

            // Required translation to bring el center to view center
            const dx = (viewCX - elCX);
            const dy = (viewCY - elCY);
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (spreadZoom.active && e.target === containerRef.current) {
          setSpreadZoom({ active: false, scale: 1, x: 0, y: 0, elementId: null });
        }
      }}
    >
