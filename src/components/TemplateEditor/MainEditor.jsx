...
=======
  // Fixed: Added underscores to unused variables to satisfy ESLint
  const { zoom, _zoomIn, _zoomOut, setZoomLevel, _fitToScreen } = useZoom(100, editorContainerRef);
...
=======
    // Fixed: Removed unused 'idx' to satisfy ESLint
    setPages(pages.map((page, i) => i === currentPage ? { ...page, html: templateHTML } : page));
>>>>>>> b2152b5 (Add updated TextEditor component with Color Stroke logic)
    setShowPreview(true);
...
=======
        pages={pages.map((page) => ({ ...page, thumbnail: getThumbnail(page.id) }))}
>>>>>>> b2152b5 (Add updated TextEditor component with Color Stroke logic)
      ...
      {showPreview && (
        <FlipbookPreview pages={pages.map(p => p.html).filter(html => html)} pageName={pageName} onClose={closePreview} isMobile={deviceInfo.isMobile} />
      )}
    </div>
  );
};
